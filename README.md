...
| EVERY1 brand | [`brand.every1movement.com`](https://brand.every1movement.com) | EVERY1's own front door, for partner organisations. Built from `every1/` in this repository. |
...
`every1/` is a second site, not a second system. It is generated by the same
`tools/build_ai.py`, checked by the same linter, and carries the same version number, so
the two cannot disagree about what the brand is. It is served from its own domain and its
own Pages project, and it holds its own copies of the stylesheet, the three faces and its
marks, so a partner organisation is never sent to `brand.theword.world` for a logo and the
site does not go dark if that domain does.

The linter knows there are two roots: a page under `every1/` resolves `/assets/...` against
`every1/assets`, not against the repository.

Fork it into its own repository the day EVERY1's palette or typefaces diverge from the
house, or the day a separate team owns it with no shared review. At that point the right
shape is EVERY1 consuming `@theword/brand` as a dependency rather than copying it.
